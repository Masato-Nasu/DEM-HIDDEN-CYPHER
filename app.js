(() => {
  ...
  const DHC_MAGIC = new Uint8Array([0x44,0x48,0x43,0x59,0x31,0x00]);    // DHCY1\0
  const PUBLIC_PASSPHRASE = 'DEM-HIDDEN-CYPHER-PUBLIC-READER-V1';
  const TYPE_TEXT = 1;
  const TYPE_URL = 2;
  const TYPE_FILE = 3;
  const TYPE_MIXED = 4;
  ...
  const I18N = {
    ja: {
      appTitle: 'DEM HIDDEN CYPHER', makeTitle: '有機証明PNGを作る', payloadType: 'データの種類', modeText: 'テキスト', modeUrl: 'URL', modeFile: 'ファイル / 画像 + 文字',
      ...
      makePassPlaceholder: '復号にも同じ合い言葉を使います', readPassPlaceholder: '作成時と同じ合い言葉 / 公開証明は空欄', pageSize: '有機ページサイズ', sizeAuto: '自動',
      makeButton: 'DEM HIDDEN CYPHERを作る', generatedTitle: '生成されたページ', saveAll: 'すべて保存（ZIP）', selfCheck: 'セルフチェック', clear: 'クリア',
      certTitle: '発行者と証明', certModeLabel: '証明モード', certModeSealed: 'Sealed Pattern / 合い言葉で本文を封印', certModePublic: 'Public Pattern / 誰でも読める公開証明',
      issuerNameLabel: '発行者名', certSubjectLabel: '件名 / 作品名 / メモ', certSubjectPlaceholder: '作品証明、招待状、秘密のURLなど', newIssuerKey: '新しい発行者鍵', exportIssuerKey: '発行者鍵を書き出す', issuerKeyPreparing: '発行者鍵を準備しています...', certNote: '内容と証明をひとつの有機模様に同居させます。公開アプリには検証用の公開鍵だけが入り、発行者の秘密鍵はこの端末に保存されます。',
      readTitle: 'DEM HIDDEN CYPHERを読む', organicPages: '有機パターンPNG', readButton: '読み出す', readPassNote: 'Public Pattern は空欄で読めます。Sealed Pattern は作成時の合い言葉が必要です。',
      ...
      makePassStatus: 'PASS: {bytes} bytes を {pages}ページのDEM HIDDEN CYPHERにしました。{size}×{size}',
    ...
    en: {
      appTitle: 'DEM HIDDEN CYPHER', makeTitle: 'Make organic proof PNG', payloadType: 'Payload type', modeText: 'Text', modeUrl: 'URL', modeFile: 'File / Image + Text',
      ...
      makePassPlaceholder: 'Use the same passphrase to read', readPassPlaceholder: 'Same passphrase used to make / leave blank for public proof', pageSize: 'Organic page size', sizeAuto: 'Auto',
      makeButton: 'Make DEM HIDDEN CYPHER', generatedTitle: 'Generated pages', saveAll: 'Save all pages (.zip)', selfCheck: 'Self check', clear: 'Clear',
      certTitle: 'Issuer and proof', certModeLabel: 'Proof mode', certModeSealed: 'Sealed Pattern / seal the text with a passphrase', certModePublic: 'Public Pattern / public proof anyone can read',
      issuerNameLabel: 'Issuer name', certSubjectLabel: 'Subject / artwork / memo', certSubjectPlaceholder: 'Artwork proof, invitation, secret URL, etc.', newIssuerKey: 'New issuer key', exportIssuerKey: 'Export issuer key', issuerKeyPreparing: 'Preparing issuer key...', certNote: 'Content and proof coexist inside one organic pattern. The public app includes only the verification public key; the issuer private key is stored on this device.',
      readTitle: 'Read DEM HIDDEN CYPHER', organicPages: 'Organic pattern PNG(s)', readButton: 'Read selected pages', readPassNote: 'Public Pattern can be read with the passphrase left blank. Sealed Pattern requires the passphrase used when it was made.',
      ...
      makePassStatus: 'PASS: {bytes} bytes issued as {pages} DEM HIDDEN CYPHER page(s), {size}×{size}.',
  ...
  const ui = {
    ...
    certMode: $('certMode'), issuerName: $('issuerName'), certSubject: $('certSubject'), certId: $('certId'),
    newIssuerKeyBtn: $('newIssuerKeyBtn'), exportIssuerBtn: $('exportIssuerBtn'), issuerKeyStatus: $('issuerKeyStatus')
  };
  ...
  function dhcTimestamp(date = new Date()) {
    return `${date.getFullYear()}${pad2(date.getMonth() + 1)}${pad2(date.getDate())}-${pad2(date.getHours())}${pad2(date.getMinutes())}${pad2(date.getSeconds())}`;
  }
  function dhcPageFilename(index = 0, total = 1, stamp = dhcTimestamp()) {
    if (total <= 1) return `DHC-${stamp}.png`;
    return `DHC-${stamp}-page-${String(index + 1).padStart(3, '0')}-of-${String(total).padStart(3, '0')}.png`;
  }
  function dhcZipFilename(total = 1, stamp = dhcTimestamp()) { return `DHC-${stamp}-${total}page.zip`; }
  ...
  function randomId(prefix='DHC') {
    const b = fillRandomBytes(new Uint8Array(6));
    return `${prefix}-${Array.from(b, x => x.toString(16).padStart(2,'0')).join('').toUpperCase()}`;
  }
  function publicIssuerId(publicJwk) {
    return 'issuer-' + fnv1a(JSON.stringify(publicJwk)).toString(16).padStart(8,'0');
  }
  function storedIssuer() {
    try { return JSON.parse(localStorage.getItem('dhc_issuer_v1') || 'null'); } catch { return null; }
  }
  ...
  async function generateIssuerKey(force=false) {
    let saved = storedIssuer();
    if (saved && !force) return saved;
    const pair = await crypto.subtle.generateKey({ name:'ECDSA', namedCurve:'P-256' }, true, ['sign','verify']);
    const privateJwk = await crypto.subtle.exportKey('jwk', pair.privateKey);
    const publicJwk = await crypto.subtle.exportKey('jwk', pair.publicKey);
    saved = { privateJwk, publicJwk, createdAt: new Date().toISOString(), issuerId: publicIssuerId(publicJwk) };
    localStorage.setItem('dhc_issuer_v1', JSON.stringify(saved));
    return saved;
  }
  async function ensureIssuerKey() {
    return generateIssuerKey(false);
  }
  async function updateIssuerKeyStatus() {
    if (!ui.issuerKeyStatus) return;
    const saved = await ensureIssuerKey();
    const label = lang === 'ja' ? '発行者鍵' : 'Issuer key';
    ui.issuerKeyStatus.textContent = `${label}: ${saved.issuerId}`;
  }
  function currentCertMode() { return ui.certMode ? ui.certMode.value : 'sealed'; }
  function currentMakePass() { return currentCertMode() === 'public' ? PUBLIC_PASSPHRASE : $('makePass').value; }
  function currentReadPass() { const v = $('readPass').value; return v ? v : PUBLIC_PASSPHRASE; }
  function updatePassUi() {
    if (!ui.certMode || !$('makePass')) return;
    const isPublic = currentCertMode() === 'public';
    $('makePass').disabled = isPublic;
    $('makePass').placeholder = isPublic ? (lang === 'ja' ? '公開証明は合い言葉なしで読めます' : 'Public certificate can be read without a passphrase') : t('makePassPlaceholder');
    if (isPublic) $('makePass').value = '';
  }
  async function makeCertificatePackage(innerPlain, payloadType) {
    const saved = await ensureIssuerKey();
    const privateKey = await importPrivateKey(saved.privateJwk);
    const payloadHash = await sha256(innerPlain);
    if (ui.certId && !ui.certId.value.trim()) ui.certId.value = randomId('DHC');
    const meta = {
      protocol: 'DEM HIDDEN CYPHER',
      version: 1,
      certificateId: (ui.certId && ui.certId.value.trim()) || randomId('DHC'),
      issuerName: (ui.issuerName && ui.issuerName.value.trim()) || 'Anonymous Issuer',
      issuerId: saved.issuerId,
      subject: (ui.certSubject && ui.certSubject.value.trim()) || '',
      mode: currentCertMode(),
      payloadType,
      issuedAt: new Date().toISOString(),
      payloadBytes: innerPlain.length,
      payloadSha256: byteHex(payloadHash),
      signatureAlgorithm: 'ECDSA-P256-SHA256',
      publicKeyJwk: saved.publicJwk
    };
    const metaBytes = enc.encode(JSON.stringify(meta));
    const toSign = concatBytes([metaBytes, innerPlain]);
    const signature = new Uint8Array(await crypto.subtle.sign({ name:'ECDSA', hash:'SHA-256' }, privateKey, toSign));
    const header = new Uint8Array(19);
    header.set(DHC_MAGIC,0); header[6] = 1;
    const view = new DataView(header.buffer);
    putU32(view,7,metaBytes.length); putU32(view,11,signature.length); putU32(view,15,innerPlain.length);
    return concatBytes([header, metaBytes, signature, innerPlain]);
  }
  async function parseCertificatePackage(bytes) {
    if (!bytesEqual(bytes.slice(0,6), DHC_MAGIC)) return parsePlainPackage(bytes);
    if (bytes.length < 19) throw new Error('DEM HIDDEN CYPHER package is too short.');
    const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const metaLen = getU32(view,7), sigLen = getU32(view,11), innerLen = getU32(view,15);
    let off = 19; const need = off + metaLen + sigLen + innerLen;
    if (need !== bytes.length) throw new Error('DEM HIDDEN CYPHER package length does not match.');
    const metaBytes = bytes.slice(off, off+metaLen); off += metaLen;
    const signature = bytes.slice(off, off+sigLen); off += sigLen;
    const innerPlain = bytes.slice(off, off+innerLen);
    let meta; try { meta = JSON.parse(dec.decode(metaBytes)); } catch { throw new Error('Certificate metadata is not valid JSON.'); }
    let signatureValid = false;
    try {
      const pub = await importPublicKey(meta.publicKeyJwk);
      signatureValid = await crypto.subtle.verify({ name:'ECDSA', hash:'SHA-256' }, pub, signature, concatBytes([metaBytes, innerPlain]));
    } catch { signatureValid = false; }
    const actualHash = byteHex(await sha256(innerPlain));
    const hashValid = actualHash === meta.payloadSha256;
    const payload = await parsePlainPackage(innerPlain);
    return { kind:'cert', meta, signatureValid: !!signatureValid, hashValid, payload };
  }
  ...
  async function makeCypher() {
    try {
      const pass=currentMakePass(); if(currentCertMode() !== 'public' && !pass) throw new Error(t('passEmpty'));
      lastOptimizeInfo=null; setStatus(ui.makeStatus,t('preparing'));
      const payload=await collectPayload();
      const innerPlain=payload.mixed ? await makeMixedPackage(payload.text,payload.fileName,payload.fileMime,payload.fileBytes) : await makePlainPackage(payload.type,payload.name,payload.mime,payload.bytes);
      const plain=await makeCertificatePackage(innerPlain, mode);
      ...
      currentBuild={pages,size:plan.size,cap:plan.cap,pass,plainBytes:plain.length,certMode:currentCertMode()}; renderOutputList(); setStatus(ui.makeStatus,t('makePassStatus',{bytes:plain.length,pages:pages.length,size:plan.size}) + (lastOptimizeInfo ? '\n' + optimizeInfoText(lastOptimizeInfo) : ''),'ok');
  ...
  async function readPages() {
    try { const pass=currentReadPass(); const files=Array.from($('readFiles').files || []); if(!files.length) throw new Error(t('choosePng')); files.sort((a,b)=>a.name.localeCompare(b.name)); setStatus(ui.readStatus,t('reading')); const chunks=[]; let size=null;
      ...
      setStatus(ui.readStatus,t('decrypting')); const bundle=concatBytes(chunks); const plain=await decryptFixedCapacity(bundle,pass); const parsed=await parseCertificatePackage(plain); await renderReadResult(parsed); setStatus(ui.readStatus,t('readPassStatus',{bytes:plain.length,pages:files.length}),'ok');
  ...
  async function renderReadResult(payload) {
    ...
    if (payload && payload.kind === 'cert') {
      const h = document.createElement('h3'); h.textContent = 'DEM HIDDEN CYPHER'; ui.readResult.appendChild(h);
      const m = payload.meta || {};
      const box = document.createElement('div'); box.className = 'decoded-text-box cert-box';
      const valid = payload.signatureValid && payload.hashValid;
      box.textContent = [
        `STATUS: ${valid ? 'VALID' : 'INVALID'}`,
        `ISSUER: ${m.issuerName || '-'}`,
        `ISSUER ID: ${m.issuerId || '-'}`,
        `CERTIFICATE ID: ${m.certificateId || '-'}`,
        `MODE: ${(m.mode || '-').toUpperCase()}`,
        `SUBJECT: ${m.subject || '-'}`,
        `ISSUED: ${m.issuedAt || '-'}`,
        `PROOF: ${payload.signatureValid ? 'SIGNATURE VERIFIED' : 'SIGNATURE FAILED'}`,
        `PAYLOAD HASH: ${payload.hashValid ? 'OK' : 'FAILED'}`
      ].join('\n');
      ui.readResult.appendChild(box);
  ...
  async function saveAllPages(){ if(!currentBuild) return; ui.downloadAllBtn.disabled=true; const old=ui.downloadAllBtn.textContent; const stamp=dhcTimestamp(); try{ const entries=[]; for(let i=0;i<currentBuild.pages.length;i++){ ui.downloadAllBtn.textContent=t('renderingAll',{i:i+1,total:currentBuild.pages.length}); const canvas=renderPageCanvas(currentBuild.pages[i],currentBuild.size,currentBuild.pass,i); const blob=await canvasToBlob(canvas); entries.push({name:dhcPageFilename(i,currentBuild.pages.length,stamp),data:await blobToBytes(blob)}); await new Promise(r=>setTimeout(r,20)); } ui.downloadAllBtn.textContent=t('buildingZip'); triggerDownload(makeStoredZip(entries),dhcZipFilename(currentBuild.pages.length,stamp)); } finally { ui.downloadAllBtn.disabled=false; ui.downloadAllBtn.textContent=old; } }
  async function selfCheck(){
    ...
    try{
      ...
      await parseCertificatePackage(plain);
  ...
  $('langJa')?.addEventListener('click',()=>{lang='ja';localStorage.setItem('tc_lang',lang);applyLanguage();updatePassUi();updateIssuerKeyStatus();}); $('langEn')?.addEventListener('click',()=>{lang='en';localStorage.setItem('tc_lang',lang);applyLanguage();updatePassUi();updateIssuerKeyStatus();});
  ui.certMode?.addEventListener('change', updatePassUi);
  ui.newIssuerKeyBtn?.addEventListener('click', async()=>{ if(confirm(lang==='ja'?'新しい発行者鍵を作ります。古い鍵で作った証明は別発行者になります。よろしいですか？':'Create a new issuer key? Certificates made with the old key will show a different issuer.')) { await generateIssuerKey(true); await updateIssuerKeyStatus(); } });
  ui.exportIssuerBtn?.addEventListener('click', async()=>{ const saved=await ensureIssuerKey(); const blob=new Blob([JSON.stringify(saved,null,2)],{type:'application/json'}); triggerDownload(blob,`dem-hidden-cypher-issuer-key-${saved.issuerId}.json`); });
  ...
  if (ui.certId && !ui.certId.value) ui.certId.value = randomId('DHC');
  applyLanguage(); setMode('text'); updatePassUi(); updateIssuerKeyStatus();
})();
